...
<template>
  <v-container fluid style="padding-top: 90px; background-color: #8BC3DC; padding-right: 0px; padding-left: 0px; padding-bottom: 0px">
                      ...
                      <!--"/assets/Screenshots(ForCommenting)/Classe/Classes-LowerSection-Top.png"-->
                        ...
                        <v-card  v-if="selectedClass != ''" style="width:100%; margin-top: 30px; margin-bottom: 30px" class="gray--text text-xs-center ml-3 mr-3">
                          <v-toolbar color="light-blue" dark height="100">
                            ...
                            <v-spacer></v-spacer>
                            <v-btn icon style="margin-right: 20px" @click="addStudents = true">
                              <v-icon large>add</v-icon>
                            </v-btn>
                          </v-toolbar>
                            ...
                            <v-flex xs12 sm6 offset-sm3>
                              <v-form>
                                <v-text-field
                                  ...
                                  required
                                ></v-text-field>
                                <v-btn @click="addStudentsWhileCreatingClass=true">Rechercher des élèves</v-btn>
                                <v-form v-if="addStudentsWhileCreatingClass">
                                  <v-select
                                    :items="theUsers"
                                    ...
                                    autocomplete
                                  ></v-select>
                                  <v-btn :disabled="addStudentName === ''" @click="addStudent(addStudentName)">Ajouter</v-btn>
                                </v-form>
    ...
    <!--//!---------------->
    <!--// ! END Create Class Dialog Section -->
    <!--//!---------------->
    ...
    <!--//!---------------->
    <!--// ! END Delete Class Dialog Section -->
    <!--//!---------------->
    ...
    <!--"/assets/Screenshots(ForCommenting)/Classe/Classes-Dialogs-AddStudents.png"-->
                      <v-dialog v-model="addStudents" fullscreen transition="dialog-bottom-transition" style="z-index: 999;position: absolute" :overlay=true
                        ...
                        <v-card>
                          <v-toolbar style="flex: 0 0 auto;" class="info">
                            <v-btn icon @click.native="addStudents = false" dark>
                              <v-icon dark>close</v-icon>
                            </v-btn>
    ...
    <!--//!---------------->
    <!--// !END Add Students Dialog Section -->
    <!--//!---------------->
    ...
    <!--"/assets/Screenshots(ForCommenting)/Classe/Classes-Dialogs-ConfirmStudentRemoval.png"-->
                      <v-layout row justify-center>
                        <v-dialog v-model="confirmStudentRemoval">
                          <v-card>
                            <v-card-title class="headline">Êtes-vous sûr de vouloir retirer l'élève?</v-card-title>
    ...
    <!--//!---------------->
    <!--// !END Confirm Student Removal Dialog Section -->
    <!--//!---------------->
...
export default {
	data() {
		return {
			createClass: false,
			createClassName: "",
			addStudentsWhileCreatingClass: false,
			addStudentName: "",
			invitedStudents: [],
	...
	computed: {
		//Whether we disable the create class button or not
		...
		//We replace the roles from and a string array to a simple string
		selectedClass() {
			if (this.$store.getters.selectedClass === "") {
				...
				for (
					var i = 0;
					i < this.$store.getters.selectedClass.users.length;
					i++
				) {
					if (
						JSON.stringify(this.$store.getters.selectedClass.users[i].roles) ===
						'["ROLE_USER"]'
					) {
	...
	methods: {
		//when we have clicked on a class
		clickedClass(aClass) {
			this.$store.dispatch("selectClass", aClass);
		},
		//We dispatch the create class method and reset our data
		createAClass(name) {
			var toSendOff = {
				name: name,
			...
			this.addStudentsWhileCreatingClass = false;
			this.invitedStudents = [];
		},
		addStudent(name) {
			for (var i = 0; i < this.users.length; i++) {
				// *!NOT GREAT IF THERE ARE STUDENTS WITH THE SAME FIRSTNAME AND SURNAME
				if (
					name ===
					this.users[i].firstname + " " + this.users[i].surname.toUpperCase()
				) {
					var userToAdd = {
						id: this.users[i].id,
	...
	created() {
		...
		//We store in an array the firstname and surname of each student
		for (var i = 0; i < this.users.length; i++) {
			if (
				JSON.stringify(this.users[i].roles) === '["ROLE_STUDENT"]' ||
				JSON.stringify(this.users[i].roles) === '["ROLE_USER"]'
			) {
